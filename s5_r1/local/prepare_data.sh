...
;;'
    # Process the STMs
    cat $exactDataDir/TEDLIUM_release1/$set/stm/*.stm | sort -k1,1 -k2,2 -k4,4n | \
  ...
  # Revised with the format string
  cat $dir/spk2utt | awk -v set=$set -v dataDir=$exactDataDir '{ printf("%s sph2pipe -f wav -p %s/TEDLIUM_release1/%s/sph/%s.sph |\n", $1, dataDir, set, $1); }' > $dir/wav.scp
